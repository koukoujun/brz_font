<template>
    <div>
        ...
        <Row :gutter="24" style="margin-bottom: 10px">
            <Col span="3">
                <Input search enter-button size="small" v-model="keyword" placeholder="输入客户姓名" @on-search="query"/>
            </Col>
            <Col span="3">
                <Input search enter-button size="small" v-model="keyworda" placeholder="输入顾问姓名" @on-search="query"/>
            </Col>
    ...
    methods: {
      ...
      getOption () {
        apiRequest(api.Common.getOption, {
          'list': 'documentary_way_list,documentary_purpose_list,department_list'
        }, ret => {
          this.option = {
      ...
      initData () {
        ...
        const keyworda = this.keyworda.trim()
        const keywordd = this.department
        //搜索关键字-开始时间
        ...
        //高端中心跟单-查看顾问历史跟单
        params.keylook = this.$route.query.id

        apiRequest(api.Documentary.getList,
          params,
          ret => {
            this.data = ret
