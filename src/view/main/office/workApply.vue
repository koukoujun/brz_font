<template>
    <div>
        ...
        <Row :gutter="24" style="margin-bottom: 10px">
            <Col span="4">
                <Input search enter-button size="small" v-model="keyword" placeholder="输入申请人" @on-search="query"/>
            </Col>
            <Col span="2">
        ...
        <Drawer title="详情" placement="left" :closable="false" v-model="showDetailDrawer">
         ...
         <!-- 简历表单-->
        <Modal v-model="formVisible" width="73" :title="formMode==='add'?'添加工作申请信息':'编辑工作申请信息'" footer-hide
          ...
          <Form :model="form" :label-width="120" :rules="formInline" ref="form" inline >
							...
							<Row>
								<Col :span="12">
									<FormItem label="申请类型" prop="category" >
											<Select v-model="form.category" size="small" style="width:8rem;" >
													<Option value="工作申请">工作申请</Option>
													<Option value="招聘申请">招聘申请</Option>
													<Option value="培训申请">培训申请</Option>
													<Option value="宣传申请">宣传申请</Option>
													<Option value="课程申请">课程申请</Option>
													<Option value="办公设备申请">办公设备申请</Option>
											</Select>
									</FormItem>
								</Col>
								<Col :span="12">
							...
							<Row>
								<Col :span="12">
									<FormItem label="申请人" prop="to_admin" >
...
<script>
  ...
  import { workApplyDetailTable, cvHeadTable} from '@/libs/data-office'
    ...
    data () {
      return {
        ...
        ChinaArea: ChinaArea,//中国行政区数据
		    imgshow:true,
				content:"write your report at here",//富文本编辑内容
        adminList:[],
				// 工作汇报表头信息
				col:[
					{
        ...
        form: {
          ...
          create_at:"",
				...
				formInline: {
					...
					apply_content: [
						{ required: true, message: '请填写申请内容', trigger: 'blur',pattern:/.+/ },
					]
				},
				...
				watch: {
					curr (_new) {
						this.currDetailTable = dataToTable(_new, workApplyDetailTable)
					}
				},
				methods: {
					...mapMutations([
        ...
        //上传开始
          ...
          this.form.to_name = this.form.to_admin.name;
           ...
           if(this.form.category=="工作申请")this.form.type=4;
           if(this.form.category=="招聘申请")this.form.type=5;
           if(this.form.category=="培训申请")this.form.type=6;
           if(this.form.category=="宣传申请")this.form.type=7;
           if(this.form.category=="课程申请")this.form.type=8;
           if(this.form.category=="办公设备申请")this.form.type=9;

          //验证
				  this.$refs[name].validate((valid) => {
					if (valid && this.to_admin!='') {

						const apiName = this.formMode === 'add' ? api.Apply.add : api.Apply.edit
      ...
      //删除简历表单
		deletes(){
			const apiName = this.formMode = api.Apply.del
