<template>
    <div>
        ...
        <Row :gutter="24" style="margin-bottom: 10px">
            <Col span="4">
                <Input search enter-button size="small" v-model="keyword" placeholder="输入汇报人" @on-search="query"/>
            </Col>
            <Col span="2">
        ...
        <Drawer title="详情" placement="left" :closable="false" v-model="showDetailDrawer">
         ...
         <!-- 简历表单-->
        <Modal v-model="formVisible" width="73" :title="formMode==='add'?'添加工作汇报信息':'编辑工作汇报信息'" footer-hide @on-cancel="form={brand_right:0}">
          <Form :model="form" :label-width="120" :rules="formInline" ref="form" inline >
							<Row :gutter="15">
							<!-- 第一行-->
							<Row >
								<Col :span="12">
									<FormItem label="汇报类型" prop="category" >
											<Select v-model="form.category" size="small" style="width:8rem;" >
													<Option value="日总结">日总结</Option>
													<Option value="周总结">周总结</Option>
													<Option value="月总结">月总结</Option>
											</Select>
									</FormItem>
								</Col>
								<Col :span="12">
									<FormItem label="汇报标题" prop="title" >
							...
							<Row>
								<Col :span="12">
									<FormItem label="汇报人" prop="to_admin"  >
								     <Select v-model="form.to_admin" size="small" style="width:8rem">
...
<script>
  ...
  export default {
    ...
    data () {
     ...
     //获取数据
      ...
      //验证
        ...
        //提交表单---------------
        if(this.form.category=="日总结")this.form.type=1;
        if(this.form.category=="周总结")this.form.type=2;
        if(this.form.category=="月总结")this.form.type=3;

				const apiName = this.formMode === 'add' ? api.Report.add : api.Report.edit
    ...
    //删除表单
		deletes(){
			const apiName = this.formMode = api.Report.del
