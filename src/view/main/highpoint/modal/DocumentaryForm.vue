<template>
  <div>
    <Modal :value="visible" class-name="vertical-center-modal" footer-hide :title="this.mode==='add'?'添加高端中心跟单记录':'编辑高端中心跟单记录'"
      @on-cancel="cancel">
...
<script>
  ...
  export default {
    ...
    props: ['visible', 'data', 'mode', 'customer_id', 'id'],
    methods: {
      getOption () {
      ...
      submit (name) {
        ...
        let apiName = this.mode === 'add' ? api.Documentary.highPointAdd : api.Documentary.edit
        if (this.mode === 'edit') {
          form.id = this.id
        }
