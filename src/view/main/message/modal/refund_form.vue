<template>
    <div>
        <Modal :value="visible" class-name="vertical-center-modal" footer-hide
               :title="this.mode==='add'?'退款回复':'退款回复'"
               @on-cancel="cancel">
			  <Form :model="form" :label-width="120" ref="form" :rules="formInline" inline  >
                <FormItem label="财务反馈" prop="feedback" >
                    <Input type="textarea" placeholder="请填写反馈内容" :autosize="{minRows: 4,maxRows: 6}" v-model="form.feedback"></Input>
                </FormItem>
                <FormItem label="退款凭证">
  ...
  export default {
    data () {
      return {
        ...
        formInline: {
         feedback: [
            { required: true, message: '请填写财务反馈', trigger: 'blur' }
            ],
         voucher_img: [
    ...
    props: ['src_id','visible','type', 'mode','id','pay_id'],
    methods: {
      //图片上传成功
      ...
      submit (name) {
       ...
       if (valid) {
           apiRequest(api.Refund.updates, this.form, ret => {
               this.$emit('success')
               //this.$Message.info(ret)
               //更新回复内容
               this.data.id=this.id
               this.data.answer=this.form.feedback
               apiRequest(api.Message.answer,this.data, ret => {});
               //更改交费记录的退款状态
               this.paydata.src_id=this.pay_id
			         this.paydata.refund_status='1'
               apiRequest(api.Payment.RefundUpdate,this.paydata,ret=>{});
